...
# Get information of Senator's parties
...
Green New Deal
```{r}
gnd <- dbGetQuery(db, "SELECT t.Username, Text, State, Hashtag FROM Tweets t INNER JOIN Senators_PI s ON t.Username = s.Username WHERE Hashtag LIKE '%GreenNewDeal%'")

gnd_text <- tibble(text = gnd$Text)

gndSentiment <- gnd_text %>% 
  unnest_tokens(word, text) %>%
  group_by(word) %>%
  summarize(count=n())
...
GND State Analysis
```{r}
gnd_text_state <- tibble(text = gnd$Text, state = gnd$State)

gndSentimentState <- gnd_text_state %>% 
  unnest_tokens(word, text) %>%
  group_by(state, word) %>%
  summarize(count=n())

gndSentimentStateAnalysis <- gndSentimentState %>% 
  inner_join(bing) %>%
...
```{r}
whm <- dbGetQuery(db, "SELECT t.Username, Text, State, Hashtag FROM Tweets t INNER JOIN Senators_PI s ON t.Username = s.Username WHERE Hashtag LIKE '%WomensHistoryMonth%'")

whm_text <- tibble(text = whm$Text)

whmSentiment <- whm_text %>% 
  unnest_tokens(word, text) %>%
  group_by(word) %>%
  summarize(count=n())
...
whm_text_state <- tibble(text = whm$Text, state = whm$State)

whmSentimentState <- whm_text_state %>% 
  unnest_tokens(word, text) %>%
  group_by(state, word) %>%
  summarize(count=n())

whmSentimentStateAnalysis <- whmSentimentState %>% 
  inner_join(bing) %>%
  group_by(state, sentiment) %>%
  summarize(score = sum(count)) %>%
  mutate(share = score/sum(score) * 100)
